...
DATA_DIR = VOLUME_ROOT / "AIA_12hour_512x512"
...
image = (
    ...
    .add_local_dir("solaris", remote_path="/root/solaris/solaris", copy=True)
)

app = modal.App(APP_NAME)
volume = modal.Volume.from_name(VOLUME_NAME, create_if_missing=True)
wandb_secrets = [modal.Secret.from_name(WANDB_SECRET_NAME)] if WANDB_SECRET_NAME else []
...
def _compute_train_scale_factors(force: bool = False) -> list[float]:
    ...
    train_path = SPLIT_DIR / "train_id.txt"
...
@app.function(
    ...
    cpu=8,
    memory=32768,
    env={"HF_HOME": str(VOLUME_ROOT / "hf-home")},
)
...
@app.function(
    ...
    env={
        "HF_HOME": str(VOLUME_ROOT / "hf-home"),
        "PYTHONPATH": "/root/solaris",
...
@app.function(image=image, volumes={VOLUME_ROOT: volume}, timeout=300)
...
@app.function(image=image, volumes={VOLUME_ROOT: volume}, timeout=600)
...
def main(
    ...
    if mode == "prepare":
        ...
        print(plot_prediction.remote(data_set=data_set, sample_index=sample_index, run_name=run_name, use_ema=use_ema))
    elif mode == "eval_mse":
